...
- If Relay is unavailable, use fallback automation engines (e.g., AutoHotkey UI automation) and report reliability trade-offs honestly.
...
- Installing/updating/removing packages, CLIs, skills, plugins, or services
- Changing OpenClaw config, channel behavior, permissions, auth, or routing
- Any command that changes runtime capabilities (what the assistant can do)
- Any setup that another agent would need to reproduce this environment
...
- Never commit raw secrets (tokens, API keys, passwords).
- Use placeholders + secret source instructions (env var, local secret store, etc.).
